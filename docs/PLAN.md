...
- 2026-07-29 — **对齐 draw.io 表格复制快捷键**：选中单元格或同一行的多个单元格后按
  Enter，深拷贝整行到其下方，保留单元格样式、文字、嵌套内容及完全位于行内的连接关系，
  跨目标行的合并块先展开以维持合法网格；Ctrl/Cmd+Enter 可从表格根或任一单元格复制整表。
  两条命令统一 fresh ids、公式/胶合重写、选中新副本与单步撤销，并确保文本输入聚焦时
  Enter / Ctrl/Cmd+Enter 仍交由编辑器处理。新增模型、控制器和应用级快捷键回归。
