...
class _DrawioXmlShapeDecoder {
      ...
      case 'text':
        ...
        if (runs.isNotEmpty) {
          _labels.add(_snapshotLabelCanvas(_DrawioStencilLabel(
            ...
            // Number(null)=0. leftover `_number` 0 per glyph so a later
            // omitted sibling does not keep TxtPinX / TxtPinY
            // (`tokens.txt` TxtPinX → svg:x; TxtPinY → svg:y).
            x: _number(node, 'x'),
            y: _number(node, 'y'),
            // mxStencil.drawNode: Number(w)*sx / Number(h)*sy; omitted is
            // Number(null)=0. leftover `_number` 0 per glyph so a later
            // omitted sibling does not keep TxtWidth / TxtHeight
            // (`tokens.txt` TxtWidth → svg:width; TxtHeight). w==0 or
            // h==0 is a tight pin, not the previous leftover cell box.
  ...
  void _applyMxCanvasRotate(XmlElement node) {
    ...
    // Number(cx)/Number(cy); omitted is Number(null)=0 so a later
    // omitted sibling rotates about the origin, not the previous
    // centre (`tokens.txt` PinX / PinY / Angle).
  ...
  void _decodePathNode(XmlElement command, List<VsdxPathCommand> commands) {
    switch (command.name.local) {
      case 'move':
        // mxStencil.drawNode: Number(getAttribute('x'))*sx /
        // Number(getAttribute('y'))*sy; omitted is Number(null)=0.
        // leftover `_number` 0 per node so a later omitted sibling does
        // not keep lastX / lastY MoveTo (`tokens.txt` Line → RelMoveTo).
      ...
      case 'line':
        ...
        // Number(getAttribute('y'))*sy; omitted is Number(null)=0.
        // leftover `_number` 0 per node so a later omitted sibling does
        // not keep lastX / lastY LineTo (`tokens.txt` Line).
        ...
        break;
      case 'curve':
        // mxStencil.drawNode: Number(x3)*sx; omitted is Number(null)=0.
        // leftover `_number` 0 per node so a later omitted sibling does
        // not keep last CubBezTo (`tokens.txt` CubBezTo → RelCubBezTo).
        _ensurePathStart(commands);
        ...
        break;
      case 'quad':
        // mxStencil.drawNode: Number(x2)*sx; omitted is Number(null)=0.
        // leftover `_number` 0 per node so a later omitted sibling does
        // not keep last QuadBezTo (`tokens.txt` RelQuadBezTo).
        _ensurePathStart(commands);
        final x1 = _number(command, 'x1');
        final y1 = _number(command, 'y1');
      ...
      case 'arc':
        ...
        // and rx/ry 0 collapses the SVG arc to LineTo. leftover
        // `_number` 0 per node so a later omitted sibling does not keep
        // CubBezTo (`tokens.txt` CubBezTo → RelCubBezTo).
        _ensurePathStart(commands);
  ...
  List<VsdxPathCommand> _decodeEllipse(XmlElement ellipse) {
    ...
    // mxGraph 0×0 <ellipse/> is a no-op like <rect/>. leftover must
    // not bake a degenerate collectEllipse sibling.
    if (width.abs() < 1e-9 && height.abs() < 1e-9) {
      return const <VsdxPathCommand>[];
    }
