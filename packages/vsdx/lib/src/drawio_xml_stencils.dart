...
class _DrawioXmlShapeDecoder {
  ...
  void _decodePathNode(XmlElement command, List<VsdxPathCommand> commands) {
    switch (command.name.local) {
      ...
      case 'quad':
        ...
        break;
      case 'arc':
        // mxStencil.drawNode: Number(x)*sx / Number(y)*sy / Number(rx)*sx /
        // Number(ry)*sy; omitted is Number(null)=0 and rx/ry 0 collapses
        // the SVG arc to LineTo even when x/y remain. leftover `_number`
  ...
  List<VsdxPathCommand> _decodeRoundRect(XmlElement rect) {
    // mxStencil.drawNode: Number(x)*sx / Number(y)*sy / Number(w)*sx /
    // Number(h)*sy; omitted is Number(null)=0. leftover `_number` 0 per
    // node so a later omitted sibling does not keep MoveTo (`tokens.txt`
    // CubBezTo → RelMoveTo).
    final left = _number(rect, 'x');
    final top = _number(rect, 'y');
  ...
  List<VsdxPathCommand> _decodeEllipse(XmlElement ellipse) {
    // mxStencil.drawNode: Number(x)*sx / Number(y)*sy / Number(w)*sx /
    // Number(h)*sy; omitted is Number(null)=0. leftover `_number` 0 per
    // node so a later omitted sibling does not keep EllipseCmd cx / cy
    // (`tokens.txt` Ellipse → svg:ellipse).
