...
void main() {
  ...
  final files = dir.listSync().whereType<File>().where((f) => f.path.endsWith('.vsd')).toList()
  ...
  for (final f in files) {
    ...
    try {
      ...
      void walk(VsdxShape s) { shapes.add(s); for (final c in s.children) walk(c); }
      for (final p in doc.pages) for (final s in p.shapes) walk(s);
      ...
      void walk2(VsdxShape s) { synthShapes.add(s); for (final c in s.children) walk2(c); }
      for (final p in synth.pages) for (final s in p.shapes) walk2(s);
